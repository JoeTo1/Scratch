...
class Blocks extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        if (!this.props.arduinoMenuClicked && nextProps.arduinoMenuClicked) {
            const runtime = this.props.vm.runtime;
            const target = runtime.getEditingTarget() || runtime.getTargetForStage();
            if (target) {
                const dynamicBlocksXML = runtime.getBlocksXML();
                const toolboxXML = makeToolboxXML(target.isStage, target.id, dynamicBlocksXML, true);
                this.props.updateToolboxState(toolboxXML);
            }
        }
        if (!this.props.uploadMenuClicked && nextProps.uploadMenuClicked) {
            axios.post('http://localhost:3000/upload').then(function (response) {
    ...
    onWorkspaceUpdate(data) {
        ...
        if (this.props.vm.editingTarget) {
            ...
            const toolboxXML = makeToolboxXML(target.isStage, target.id, dynamicBlocksXML, this.props.arduinoMenuClicked);
            this.props.updateToolboxState(toolboxXML);
        }
...
Blocks.propTypes = {
    ...
    arduinoMenuClicked: PropTypes.bool
};
...
const mapStateToProps = state => ({
    ...
    arduinoMenuClicked: state.scratchGui.menus.arduinoMenuClicked,
    uploadMenuClicked: state.scratchGui.menus.uploadMenuClicked
});
